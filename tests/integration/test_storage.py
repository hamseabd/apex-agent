...
def test_compounds_start_date_excluded_from_yaml(s3_bucket):
    ...
    assert "start_date" not in compound_section, (
        "compound start_date=None should be excluded by exclude_none=True"
    )
...
def test_load_raises_protocol_not_found_when_missing(s3_bucket):
    from apex.infra.storage import ProtocolNotFoundError, ProtocolStore
